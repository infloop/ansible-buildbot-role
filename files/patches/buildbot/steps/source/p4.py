...
class P4(Source):
  ...
  renderables = ['mode', 'p4base', 'p4client', 'use_streams', 'p4stream', 'p4viewspec', 'p4client_spec_options', 'p4branch']
  possible_modes = ('incremental', 'full')

  def __init__(self, mode='incremental',
               method=None, p4base=None,
               p4stream=None, p4branch=None,
    ...
    if not p4viewspec and p4base is None and use_streams is None:
      config.error("You must provide p4base or p4viewspec or use_streams")

    if use_streams and p4stream is None:
      config.error(
        "You must provide p4stream when using streams (use_streams = True)")

    if use_streams and (p4viewspec or p4branch or p4extra_views):
      config.error(
        "Either use streams (use_streams = True) or p4base and p4branch (and optionally p4extra_views")
  ...
  def mode_incremental(self, _):
    ...
    syncStdout = yield self._dovccmd(command, collectStdout=True)

    self.updateSourceProperty('got_p4_sync_stdout', syncStdout)
  ...
  def _dovccmd(self, command, collectStdout=False, initialStdin=None):
    ...
    log.msg("P4:_dovccmd():workdir->%s" % self.workdir)
    log.msg("P4:_dovccmd():command->%s" % command)
    log.msg("P4:_dovccmd():initialStdin->%s" % initialStdin)
