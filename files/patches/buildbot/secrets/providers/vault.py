...
class HashiCorpVaultSecretProvider(SecretProviderBase):
  ...
  def get(self, entry):
    ...
    """
    secretsmount = self.secretsmount
    if secretsmount.endswith('/'):
      secretsmount = secretsmount[:-1]

    if self.apiVersion == 1:
      path = secretsmount
    else:
      path = secretsmount + '/data'
    ...
    httpPath = '/v1/{0}'.format(path)
    proj = yield self._http.get(httpPath)
    code = yield proj.code
    if code != 200:
      raise KeyError("The key %s does not exist in Vault provider in http path %s: request"
                     " return code:%d." % (entry, httpPath, code))
    json = yield proj.json()
    if self.apiVersion == 1:
      ret = json.get('data', {}).get(entry)
    else:
      ret = json.get('data', {}).get('data', {}).get(entry)
    return ret
